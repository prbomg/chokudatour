...
class FixturePDO extends PDO
{
    public function __construct(bool $legacy = false)
    {
        ...
        $this->exec("CREATE TABLE blocked_dates (id INTEGER PRIMARY KEY, block_date TEXT, tour_ids TEXT, reason TEXT)");
        ...
        $this->exec("INSERT INTO global_settings (setting_key,setting_value) VALUES ('admin_sync_token','fixture-token')");
    ...
    private function translate(string $sql): string
    {
        ...
        return str_replace([" SEPARATOR '||'", 'CURDATE() - INTERVAL 15 DAY'], [", '||'", "date(CURDATE(), '-15 days')"], $sql);
    }
...
$current_user_name = 'Тестовый администратор';
$_SESSION = ['user_name' => $current_user_name, 'user_role' => 'admin'];
