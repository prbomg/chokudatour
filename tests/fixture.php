...
class FixturePDO extends PDO
{
    public function __construct(bool $legacy = false)
    {
        ...
        $this->exec("CREATE TABLE guide_timeoffs (id INTEGER PRIMARY KEY, guide_name TEXT, guide_id INT, date_off TEXT, reason TEXT)");
        ...
        $this->exec("INSERT INTO users (id,name,email,password,role,guide_id) VALUES (1,'Тестовый администратор','admin@example.invalid','hash','admin',NULL), (2,'Гид А','guide@example.invalid','hash','guide',1)");
        $this->exec("INSERT INTO events (id,tour_date,tour_id,guide,guide_id,notes) VALUES (1,'2026-09-05',1,'Гид А',1,'ОченьДлинноеПримечаниеБезПробеловДляПроверкиПереносаНаМобильномЭкране'), (2,'2026-09-06',2,'Гид Б',2,''), (3,'2026-10-05',1,'Гид Б',2,''), (4,'2026-08-01',1,'Гид А',1,''), (5,'2026-09-07',1,'Гид А',1,'Без туристов, с расходами')");
...
$current_user_guide_id = $GLOBALS['fixture_guide_id'] ?? ($current_user_role === 'guide' ? 1 : null);
$_SESSION = ['user_id' => $current_user_id, 'user_name' => $current_user_name, 'user_role' => $current_user_role, 'guide_id' => $current_user_guide_id, 'form_token' => 'fixture-token'];
