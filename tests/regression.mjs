...
try {
    ...
    const response = await php.run({code: `<?php
      ...
      register_shutdown_function(function() {
        ...
        if (isset($GLOBALS['pdo'])) {
          ...
          $data['global_settings'] = $GLOBALS['pdo']->query('SELECT * FROM global_settings ORDER BY setting_key')->fetchAll();
  ...
  assert.match(clientsExport.headers['content-disposition'][0], /clients_.*\.csv/);
  assert.ok(clientsExport.html.includes('Телефон;'));
  assert.ok(clientsExport.html.includes('+7 000 000-00-00'));
  checks++;
  const eventsExport = await page('backup.php', {}, {backup_action:'events'});
  assert.ok(eventsExport.html.includes('Маршрут;Гид'));
  assert.ok(eventsExport.html.includes('Длинное название экскурсии'));
  checks++;
  const paymentsExport = await page('backup.php', {}, {backup_action:'payments'}, false, {setup:["INSERT INTO payments (event_id,participant_id,operation,amount,method,paid_at,note) VALUES (1,1,'payment',500,'cash','2026-09-05','Тест')"]});
  assert.ok(paymentsExport.html.includes('"Дата операции";"Дата выезда"'));
  assert.ok(paymentsExport.html.includes(';500;cash;'));
  checks++;
  const databaseBackup = await page('backup.php', {}, {backup_action:'database'});
  assert.match(databaseBackup.headers['content-disposition'][0], /chokudatour_database_.*\.sql/);
  assert.ok(databaseBackup.html.includes('CREATE TABLE events'));
  assert.ok(databaseBackup.html.includes('INSERT INTO `participants`'));
  assert.ok(databaseBackup.data.global_settings.some(row => row.setting_key === 'last_database_backup_at'));
  checks++;
  const receiptsBackup = await page('backup.php', {}, {backup_action:'receipts'});
  assert.match(receiptsBackup.headers['content-disposition'][0], /receipts_.*_0-files\.zip/);
  assert.ok(receiptsBackup.html.startsWith('PK'));
  checks++;
  const forbiddenBackup = await page('backup.php', {}, {backup_action:'clients'}, false, {role:'guide'});
  assert.equal(forbiddenBackup.status, 403);
  checks++;
