...
$date_col = 'date';
if(in_array('event_date', $events_cols)) $date_col = 'event_date';
if(in_array('tour_date', $events_cols)) $date_col = 'tour_date';

$guide_col = 'guide_id';
if(in_array('guide', $events_cols)) $guide_col = 'guide';
...
// --- СМЕНА ПАРАМЕТРОВ ЭКСКУРСИИ (АДМИН) ---
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_participant'])) {
    ...
    // Проверка колонки для имени (name или client_name)
    $name_col = in_array('client_name', $part_cols) ? 'client_name' : 'name';
    
    // Токен (если есть)
    $token_sql = ""; $token_val = [];
    if(in_array('ticket_token', $part_cols)) {
        $token_sql = ", ticket_token";
        $token_val = [substr(md5(uniqid(rand(), true)), 0, 32)];
    }
...
<head>
    ...
    <style>
        ...
        .guide-select-box input[type="date"], .guide-select-box input[type="time"], .guide-select-box select { padding: 8px 14px; border-radius: var(--radius-sm); border: 1px solid #C7D2FE; font-size: 14px; font-family: inherit; outline: none; background: white; font-weight: 600; color: var(--text-main); cursor: pointer; transition: var(--transition); }
        .guide-select-box input:hover, .guide-select-box select:hover { border-color: var(--primary); }
        .btn-update-details { background: var(--primary); color: white; border: none; padding: 8px 16px; border-radius: var(--radius-sm); font-weight: 700; cursor: pointer; transition: var(--transition); font-size: 13px; margin-left: 10px;}
        .btn-update-details:hover { background: var(--primary-hover); transform: translateY(-1px); }
        ...
        input.t-input, select.t-input { width: 100%; box-sizing: border-box; padding: 9px 12px; background: #F8FAFC; border: 1px solid transparent; border-radius: var(--radius-sm); font-size: 13px; font-family: inherit; outline: none; transition: var(--transition); color: var(--text-main); font-weight: 500;}
        ...
        @media (max-width: 768px) {
            ...
            .guide-select-box { flex-direction: column; align-items: stretch; }
            .btn-update-details { margin-left: 0; margin-top: 10px;}
        }
    </style>
</head>
<body>
        ...
        <?php if ($current_user_role === 'admin'): ?>
            <form method="POST" style="margin:0;" class="guide-select-box">
                ...
                <div style="display:flex; align-items:center; gap:6px;">
                    <label>Дата:</label>
                    <input type="date" name="tour_date" value="<?= htmlspecialchars($event[$date_col]) ?>" required>
                </div>

                <div style="display:flex; align-items:center; gap:6px;">
                    <label>Время:</label>
                    <input type="time" name="time" value="<?= htmlspecialchars($event[$time_col] ?? '') ?>">
                </div>

                <div style="display:flex; align-items:center; gap:6px;">
                    <label>Гид:</label>
                    <select name="guide">
                        <option value="Не назначен" <?= ($event[$guide_col] ?? '') === 'Не назначен' ? 'selected' : '' ?>>Не назначен</option>
                        <?php foreach ($guides as $g): ?>
                            <option value="<?= htmlspecialchars($g ?? '') ?>" <?= ($event[$guide_col] ?? '') === $g ? 'selected' : '' ?>><?= htmlspecialchars($g ?? '') ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <button type="submit" class="btn-update-details">Сохранить</button>
            </form>
        <?php else: ?>
            <div class="guide-select-box">
                <div style="display:flex; align-items:center; gap:6px;">
                    <label>Время старта:</label>
                    <span style="font-size:14px; font-weight:700; color:var(--text-main);"><?= htmlspecialchars($event[$time_col] ?: 'Не указано') ?></span>
                </div>
                <div style="display:flex; align-items:center; gap:6px;">
                    <label>Гид:</label>
                    <span style="font-size:14px; font-weight:700; color:var(--text-main);">🙋‍♂️ <?= htmlspecialchars($event[$guide_col] ?? '') ?></span>
                </div>
            </div>
        <?php endif; ?>
    </div>
            ...
            <table>
                ...
                <tbody>
                        ...
                        <td>
                            <select form="formAddParticipant" name="source" class="t-input">
                                <?php foreach ($sources_list as $src): ?>
                                    <option value="<?= htmlspecialchars($src) ?>" <?= $src === 'CRM' ? 'selected' : '' ?>><?= htmlspecialchars($src) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td>
                    ...
                    <tr class="edit_form_row edit_p_<?= $p_id ?>" style="display: none;">
                        <td><input form="formEditP_<?= $p_id ?>" type="text" name="client_name" class="t-input" value="<?= htmlspecialchars($p_name) ?>" required></td>
                        <td class="contact-col">
                        ...
                        <td>
                            <select form="formEditP_<?= $p_id ?>" name="source" class="t-input">
                                <?php foreach ($sources_list as $src): ?>
                                    <option value="<?= htmlspecialchars($src) ?>" <?= ($p['source'] ?? '') === $src ? 'selected' : '' ?>><?= htmlspecialchars($src) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                        <td>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (msg) {
            const messages = {
                'event_updated': 'Параметры экскурсии сохранены!',
