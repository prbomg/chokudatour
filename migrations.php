...
function applicationMigrations(): array
{
    return [
        1 => ['name'=>'Базовые таблицы сервиса','up'=>function(PDO $pdo): void {
            ...
            migrationCreate($pdo,'participants',"CREATE TABLE participants (id INT AUTO_INCREMENT PRIMARY KEY,event_id INT NOT NULL,client_name VARCHAR(255) NOT NULL,phone VARCHAR(50) DEFAULT '',email VARCHAR(100) DEFAULT '',seats INT DEFAULT 1,places INT DEFAULT 1,price DECIMAL(12,2) DEFAULT 0,source VARCHAR(255),status VARCHAR(100),notes TEXT,ticket_token VARCHAR(64))","CREATE TABLE participants (id INTEGER PRIMARY KEY AUTOINCREMENT,event_id INT NOT NULL,client_name TEXT NOT NULL,phone TEXT DEFAULT '',email TEXT DEFAULT '',seats INT DEFAULT 1,places INT DEFAULT 1,price DECIMAL(12,2) DEFAULT 0,source TEXT,status TEXT,notes TEXT,ticket_token TEXT)");
            ...
            migrationCreate($pdo,'expenses',"CREATE TABLE expenses (id INT AUTO_INCREMENT PRIMARY KEY,event_id INT NOT NULL,category VARCHAR(100) DEFAULT 'Прочее',amount DECIMAL(12,2) NOT NULL,description TEXT)","CREATE TABLE expenses (id INTEGER PRIMARY KEY AUTOINCREMENT,event_id INT NOT NULL,category TEXT DEFAULT 'Прочее',amount DECIMAL(12,2) NOT NULL,description TEXT)");
        }],
        2 => ['name'=>'Справочники и служебные таблицы','up'=>function(PDO $pdo): void {
            ...
            migrationCreate($pdo,'payments',"CREATE TABLE payments (id INT AUTO_INCREMENT PRIMARY KEY,event_id INT NOT NULL,participant_id INT NOT NULL,operation VARCHAR(20) NOT NULL DEFAULT 'payment',amount DECIMAL(12,2) NOT NULL,method VARCHAR(30) NOT NULL DEFAULT 'cash',paid_at DATE NOT NULL,note VARCHAR(500) DEFAULT '',created_by VARCHAR(100) DEFAULT '',created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,voided_at DATETIME,voided_by VARCHAR(100))","CREATE TABLE payments (id INTEGER PRIMARY KEY AUTOINCREMENT,event_id INT NOT NULL,participant_id INT NOT NULL,operation TEXT NOT NULL DEFAULT 'payment',amount DECIMAL(12,2) NOT NULL,method TEXT NOT NULL DEFAULT 'cash',paid_at TEXT NOT NULL,note TEXT DEFAULT '',created_by TEXT DEFAULT '',created_at TEXT DEFAULT CURRENT_TIMESTAMP,voided_at TEXT,voided_by TEXT)");
        }],
        ...
        7 => ['name'=>'Денежные суммы с точностью до копеек','up'=>function(PDO $pdo): void {
            if (migrationDriver($pdo)==='mysql') {
                $pdo->exec('ALTER TABLE participants MODIFY price DECIMAL(12,2) NOT NULL DEFAULT 0.00');
                $pdo->exec('ALTER TABLE expenses MODIFY amount DECIMAL(12,2) NOT NULL');
                $pdo->exec('ALTER TABLE payments MODIFY amount DECIMAL(12,2) NOT NULL');
            }
        }],
    ];
}
