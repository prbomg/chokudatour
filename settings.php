...
<head>
    ...
    <style>
        ...
        /* Критические стили блока копий находятся в странице, чтобы кэш внешнего CSS не ломал интерфейс. */
        .backup-section{overflow:hidden;border:1px solid var(--border);border-radius:18px;background:#fff;box-shadow:var(--shadow-sm)}
        .backup-header{display:flex;align-items:center;justify-content:space-between;gap:28px;padding:22px 24px;border-bottom:1px solid var(--border)}
        .backup-header>div:first-child{max-width:760px}.backup-header h2{margin:0;font-size:24px;letter-spacing:-.035em}.backup-header p{margin:6px 0 0;color:var(--text-muted);font-size:13px;line-height:1.5}
        .backup-status{min-width:250px;padding:12px 14px;border:1px solid #ead9ac;border-radius:12px;background:#fff8e6}.backup-status.is-ready{border-color:#cfe0d8;background:var(--primary-light)}.backup-status span,.backup-status small{display:block;color:var(--text-muted);font-size:10px}.backup-status strong{display:block;margin:4px 0;color:var(--text-main);font-size:14px}.backup-status small{overflow-wrap:anywhere}
        .backup-body{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(300px,.75fr);gap:16px;padding:18px;background:#f8faf9}.export-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
        .export-card{display:grid;grid-template-columns:42px minmax(0,1fr) auto;align-items:center;gap:12px;min-width:0;margin:0;padding:15px;border:1px solid var(--border);border-radius:13px;background:#fff}.backup-icon,.database-icon{display:inline-flex;font-size:11px;font-weight:850;letter-spacing:-.02em;align-items:center;justify-content:center;width:42px;height:42px;border-radius:11px;background:var(--primary-light);color:var(--primary)}
        .export-card>div{display:grid;gap:3px;min-width:0}.export-card strong{font-size:14px}.export-card div span{color:var(--text-muted);font-size:10px;line-height:1.35}
        .export-card button{display:inline-flex;align-items:center;gap:7px;min-height:34px;padding:7px 10px;border:1px solid #c7d8d0;border-radius:9px;background:#fff;color:var(--primary);font:inherit;font-size:11px;font-weight:800;cursor:pointer}.export-card button b,.database-backup button b{font-size:15px;font-weight:500}
        .database-backup{display:flex;flex-direction:column;align-items:flex-start;min-width:0;margin:0;padding:19px;border-radius:14px;background:var(--primary);color:#fff}.database-icon{margin-bottom:16px;background:rgba(255,255,255,.13);color:#fff}.database-backup>div{display:grid;gap:5px}.backup-kicker{color:rgba(255,255,255,.66);font-size:9px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.database-backup strong{font-size:18px}.database-backup p{margin:2px 0 16px;color:rgba(255,255,255,.72);font-size:11px;line-height:1.45}
        .database-backup button{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;min-height:40px;margin-top:auto;padding:9px 12px;border:0;border-radius:9px;background:#fff;color:var(--primary);font:inherit;font-size:11px;font-weight:800;cursor:pointer}.database-backup>small{align-self:center;margin-top:8px;color:rgba(255,255,255,.58);font-size:9px}
        @media(max-width:1000px){.backup-body{grid-template-columns:1fr}.database-backup{display:grid;grid-template-columns:48px 1fr auto;align-items:center;gap:15px}.database-icon{margin:0}.database-backup p{margin-bottom:0}.database-backup button{width:auto;margin:0}.database-backup>small{display:none}}
        @media(max-width:760px){.backup-header{align-items:flex-start}.export-grid{grid-template-columns:1fr}.database-backup{display:flex;align-items:flex-start}.database-backup button{width:100%;margin-top:5px}}
        @media(max-width:650px){.backup-header{display:grid;padding:18px}.backup-status{min-width:0}.backup-body{padding:10px}.export-card{grid-template-columns:38px minmax(0,1fr) auto;padding:12px}.backup-icon{width:38px;height:38px}.database-backup{padding:17px}}
    </style>
    ...
    <section class="backup-section section-wrap" aria-labelledby="backup-title">
        ...
        <div class="backup-body">
            <div class="export-grid">
                <form method="post" action="backup.php" class="export-card"><?= formTokenInput() ?><input type="hidden" name="backup_action" value="clients"><span class="backup-icon" aria-hidden="true">КЛ</span><div><strong>Клиенты</strong><span>Контакты, поездки, суммы, теги и заметки</span></div><button type="submit">CSV <b>↓</b></button></form>
                <form method="post" action="backup.php" class="export-card"><?= formTokenInput() ?><input type="hidden" name="backup_action" value="events"><span class="backup-icon" aria-hidden="true">В</span><div><strong>Выезды</strong><span>Расписание, гиды, места, доходы и расходы</span></div><button type="submit">CSV <b>↓</b></button></form>
                <form method="post" action="backup.php" class="export-card"><?= formTokenInput() ?><input type="hidden" name="backup_action" value="payments"><span class="backup-icon" aria-hidden="true">₽</span><div><strong>Платежи</strong><span>Оплаты и возвраты с привязкой к туристам</span></div><button type="submit">CSV <b>↓</b></button></form>
                <form method="post" action="backup.php" class="export-card"><?= formTokenInput() ?><input type="hidden" name="backup_action" value="receipts"><span class="backup-icon" aria-hidden="true">ZIP</span><div><strong>Архив чеков</strong><span>ZIP с файлами и реестром расходов</span></div><button type="submit">ZIP <b>↓</b></button></form>
            </div>
            <form method="post" action="backup.php" class="database-backup" onsubmit="return confirm('Создать полную резервную копию базы? Сохраните скачанный файл в надёжном месте.')"><?= formTokenInput() ?><input type="hidden" name="backup_action" value="database"><span class="database-icon" aria-hidden="true">DB</span><div><span class="backup-kicker">Полное сохранение</span><strong>Резервная копия базы</strong><p>Все выезды, туристы, платежи, настройки и структура сервиса в одном SQL-файле.</p></div><button type="submit">Создать и скачать копию <b>↓</b></button><small>Храните файл в защищённом месте</small></form>
        </div>
    </section>
