...
<div class="container">
    ...
    <main class="profile-page">
        ...
        <section class="profile-hero">
            <div class="profile-main"><span class="eyebrow">Карточка клиента</span><h1><?= htmlspecialchars($client_name) ?></h1><div class="profile-contacts"><a href="tel:<?= htmlspecialchars($phone, ENT_QUOTES) ?>"><?= htmlspecialchars($phone) ?></a><?php if ($client_email): ?><a href="mailto:<?= htmlspecialchars($client_email, ENT_QUOTES) ?>"><?= htmlspecialchars($client_email) ?></a><?php endif; ?></div>
                <div class="contact-actions"><a class="btn btn-primary" href="tel:<?= htmlspecialchars($phone, ENT_QUOTES) ?>">Позвонить</a><a class="btn btn-whatsapp" href="https://wa.me/<?= htmlspecialchars($clean_phone, ENT_QUOTES) ?>" target="_blank">WhatsApp</a><a class="btn btn-telegram" href="https://t.me/+<?= htmlspecialchars($clean_phone, ENT_QUOTES) ?>" target="_blank">Telegram</a><?php if ($client_email): ?><a class="btn btn-quiet" href="mailto:<?= htmlspecialchars($client_email, ENT_QUOTES) ?>">Написать</a><?php endif; ?><button type="button" class="icon-button" id="copyPhone" data-phone="<?= htmlspecialchars($phone, ENT_QUOTES) ?>" title="Скопировать телефон" aria-label="Скопировать телефон"><svg><use href="#i-copy"/></svg></button></div>
            </div>
        ...
        <div class="profile-grid">
            <section class="profile-card profile-settings">
                <header><div><span class="eyebrow">Сегментация</span><h2>Профиль клиента</h2></div><button type="button" class="icon-button" data-open-dialog="tagManagerDialog" title="Управление общими тегами" aria-label="Управление общими тегами"><svg><use href="#i-settings"/></svg></button></header>
...
<script>window.clientPageConfig=<?= json_encode(['message'=>$_GET['msg'] ?? '', 'error'=>$page_error], JSON_UNESCAPED_UNICODE|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT) ?>;</script><script src="assets/client-workspace.js"></script>
</body></html>
